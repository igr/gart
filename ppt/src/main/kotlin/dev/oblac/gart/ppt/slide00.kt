...
private val stroke = strokeOf(NipponColors.col029_GINSYU, 8f).apply {
    this.imageFilter = ImageFilter.makeDropShadow(20f, 20f, 20f, 20f, NipponColors.col044_BENIHI)
}
...
val slide00 = DrawFrame { c, d, f ->
    ...
    forSequence(yForm.i(), yTo.i(), 20).forEachIndexed { i, y ->
        ...
        for (x in 0 until d.w step step) {
            ...
            if (x == 0) {
                path.moveTo(Point(x.f() * step, py))
            } else {
                path.lineTo(Point(x.f() * step, py))
            }
        }
        val pd = path.detach()
