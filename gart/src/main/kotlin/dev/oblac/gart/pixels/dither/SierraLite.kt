...
fun ditherSierraLite(bitmap: Pixels, pixelSize: Int = 1, colorCount: Int = 256) {
    ...
    for (y in 0 until height step pixelSize) {
        for (x in 0 until width step pixelSize) {
            ...
            if (rightX < width) 
                bitmap.addBlockColor(rightX, y, pixelSize, (errorR * 2.0 / 4.0).toInt(), (errorG * 2.0 / 4.0).toInt(), (errorB * 2.0 / 4.0).toInt())
            
            if (bottomY < height) {
                if (bottomLeftX >= 0) 
                    bitmap.addBlockColor(bottomLeftX, bottomY, pixelSize, (errorR * 1.0 / 4.0).toInt(), (errorG * 1.0 / 4.0).toInt(), (errorB * 1.0 / 4.0).toInt())
            
                bitmap.addBlockColor(x, bottomY, pixelSize, (errorR * 1.0 / 4.0).toInt(), (errorG * 1.0 / 4.0).toInt(), (errorB * 1.0 / 4.0).toInt())
            }
        }
    }
}
