...
fun ditherTwoRowSierra(bitmap: Pixels, pixelSize: Int = 1, colorCount: Int = 256) {
    ...
    for (y in 0 until height step pixelSize) {
        for (x in 0 until width step pixelSize) {
            ...
            // Distribute error using Two-Row Sierra weights
            ...
            if (rightX < width) 
                bitmap.addBlockColor(rightX, y, pixelSize, (errorR * 4.0 / 16.0).toInt(), (errorG * 4.0 / 16.0).toInt(), (errorB * 4.0 / 16.0).toInt())
            
            if (rightX2 < width) 
                bitmap.addBlockColor(rightX2, y, pixelSize, (errorR * 3.0 / 16.0).toInt(), (errorG * 3.0 / 16.0).toInt(), (errorB * 3.0 / 16.0).toInt())
            
            if (bottomY < height) {
                if (bottomLeftX2 >= 0) 
                    bitmap.addBlockColor(bottomLeftX2, bottomY, pixelSize, (errorR * 1.0 / 16.0).toInt(), (errorG * 1.0 / 16.0).toInt(), (errorB * 1.0 / 16.0).toInt())
                
                if (bottomLeftX >= 0) 
                    bitmap.addBlockColor(bottomLeftX, bottomY, pixelSize, (errorR * 2.0 / 16.0).toInt(), (errorG * 2.0 / 16.0).toInt(), (errorB * 2.0 / 16.0).toInt())
            
                bitmap.addBlockColor(x, bottomY, pixelSize, (errorR * 3.0 / 16.0).toInt(), (errorG * 3.0 / 16.0).toInt(), (errorB * 3.0 / 16.0).toInt())
            
                if (bottomRightX < width) 
                    bitmap.addBlockColor(bottomRightX, bottomY, pixelSize, (errorR * 2.0 / 16.0).toInt(), (errorG * 2.0 / 16.0).toInt(), (errorB * 2.0 / 16.0).toInt())
                
                if (bottomRightX2 < width) 
                    bitmap.addBlockColor(bottomRightX2, bottomY, pixelSize, (errorR * 1.0 / 16.0).toInt(), (errorG * 1.0 / 16.0).toInt(), (errorB * 1.0 / 16.0).toInt())
            }
        }
    }
}
