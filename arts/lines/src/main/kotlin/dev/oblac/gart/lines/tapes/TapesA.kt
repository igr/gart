...
fun main() {
    val gart = Gart.of("tapesA", 1024, 1024)
    println(gart)
...
private fun draw(c: Canvas, d: Dimension) {
    ...
    generateOverlappingClosedPath(7, radius = 500f, centerX = 512f, centerY = 512f).let { lines ->
...
private fun drawGrungePoly4v2(c: Canvas, poly: Poly4, color: Int) {
    ...
    repeat(2000) {
        ...
        c.drawLine(
            ...
            strokeOf(color, 0.3f).apply {
                this.alpha = 40
                this.pathEffect = PathEffect.makeDiscrete(10f, 30f, currentNanoTime().toInt())
            }
        )
    }
...
private fun drawGrungePoly4v3(c: Canvas, poly: Poly4, color: Int) {
    val lines = poly.lines().sortedBy { it.length() }
    val sideOne = lines[2]
    val sideTwo = lines[3]

    val sideOnePoints = sideOne.points(200)
    val sideTwoPoints = sideTwo.points(200).reversed()

    sideOnePoints.zip(sideTwoPoints)
        .map { (a, b) -> Line(a, b) }
        .filterIndexed { i, _ ->
            val sin = sin(i / 10f)
            Math.random() < (sin + 1) / 2
        ...
        .forEach {
            repeat(10) { r ->
                c.drawLine(
                    ...
                    strokeOf(color, 4.8f).apply {
                        this.alpha = 10
                        this.pathEffect = PathEffect.makeDiscrete(8f, 8f, currentNanoTime().toInt())
                    }
                )
            }
        }
